...
def load_frames(file: str) -> dict:
    ...
    for j in range(time_step):
        for k in range(n_sections):
            ...
            if k == section: # only the annulus section
                a_sum = np.sum(c_vals, axis=0)
                a_sum[a_sum == 0] = 1 # replace all zero-sums with 1
                w_c_vals: np.ndarray = np.sum(base_colors * c_vals[..., np.newaxis], axis=0) / a_sum[..., np.newaxis]
    ...
    # collect all frames
    print("pipe_frames.shape", pipe_frames[0].shape, "ann_frames.shape", ann_frames[0].shape )
    frames = np.vstack([pipe_frames, ann_frames])
